      ...
      <div className="flex gap-6">
        <div className="flex flex-col m-4 gap-6">
          {steps.map((value, index) => (
            <Step num={index + 1} key={index}>{value}</Step>
          ))}
        </div>
...
const Step = ({
  ...
  return (
    <div className="relative">
      <div className="p-6 text-3xl bg-sky-600 rounded-3xl">
        {children}
      </div>
      <Tail/>
    </div>
...
const Tail = () => {
  return <div className='w-5 h-5 left-[-5px] bottom-[2px] shadow-[6px_4px_0_0] shadow-sky-600 bottom-0 absolute' style={{
    borderRadius: '50%',
  }}/>
}